...
def write_minimal_state(root: Path, device: str) -> None:
    ...
    (root / "pantsu_overrides.tsv").write_text("version\t2\n", encoding="utf-8")
    (root / "pantsu_candidate_order.tsv").write_text(
        "version\t2\n", encoding="utf-8")
    (root / "pantsu_self_words.tsv").write_text("version\t1\n", encoding="utf-8")
    ...
    (root / "pantsu_usage_events.tsv").write_text(
        "version\t1\n", encoding="utf-8")
    (root / "pantsu_history.tsv").write_text("", encoding="utf-8")
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        os.utime(root / "pantsu_usage.tsv", ns=(10, 10))
        os.utime(sync / "pantsu_usage.tsv", ns=(20, 20))
        maintenance.sync_state_directory()
        assert "同步词" in (root / "pantsu_usage.tsv").read_text(
            encoding="utf-8")
        assert "同步词" in (sync / "pantsu_usage.tsv").read_text(
            encoding="utf-8")
