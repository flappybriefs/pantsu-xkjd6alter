...
python3 tools/pantsu_maintenance.py logs
python3 tools/pantsu_maintenance.py restore-log
```
...
2. 按“操作时间、文件修改时间、设备标识”的顺序选取最新操作；
3. 先自动备份，再把合并结果写回电脑；
4. 同一份结果双向写回所有参与合并的电脑和手机状态目录；
5. 重新加载鼠须管。

仓输入法把应用文件与键盘运行目录分开。`overrideDictFiles: false` 会在同步
或重新部署前先用键盘文件覆盖应用文件，适合把手机新操作上传，却会阻止
电脑结果进入键盘。因此完整流程是：

1. 手机上保持“重新部署时覆盖词库文件”关闭，执行同步，上传手机新操作；
2. 电脑运行维护工具合并；
3. 手机等待 iCloud 下载完成，临时开启“重新部署时覆盖词库文件”；
4. 执行重新部署，让应用中的合并结果覆盖键盘运行目录；
5. 部署完成后立即关闭该选项，继续保护手机运行时数据。

Lua 只读取键盘运行目录，不会直接读取 iCloud 文件，因此第 3—4 步不可省略。

覆盖、自造词和候选顺序发生竞争时，最新平台的操作胜出；时间精度相同则
使用文件实际修改时间，仍相同才用设备标识稳定决胜。所有竞争记录都会写入
`pantsu_sync_conflicts.tsv`，可以查看哪一方胜出。

`pantsu_history.tsv` 采用集合合并：手机和电脑的不同历史全部保留，完全相同
的行去重，再按时间排序，并继续遵守 1MB 上限，不再用电脑整文件覆盖手机。

每次合并前会在 `pantsu_maintenance_logs/` 保存恢复日志，只保留最近 5 次。
双击 `胖次键道恢复.command`，选择序号并输入 `YES`，即可把电脑和手机共享
目录恢复到该次合并前。
...
维护工具“更多功能 → 对比自造词写入性能”会按 50、100、250、500KB
生成临时自造词数据，用真实 Lua 分别测量胖次完整快照更新、天行键删除和
天行键替换/前移 snapshot。测试模拟日常操作时缓存已经加载的状态，不把
首次读取文件混进写入耗时。测试只操作系统临时目录，不会修改真实词库。
也可以直接运行：

```bash
python3 tools/pantsu_maintenance.py self-benchmark
```
...
自造词权威快照仍采用原子写入和回读校验，但内存中持续维护已排序的键列表；
日常新增、删除或改码不再先对整张自造词表重新排序。这样保留完整快照的
可靠性，同时消除词表变大后的重复整表排序成本。
