...
- 无候选时单击[开启自造词，**自造词时建议逐字打，打词也可，最好不要用630** ，最后按空格确认上屏后自造词完成，自造词保存在pantsu.user.dict最后，无需重新部署即可以直接使用；
...
- 890按键现用作调频，将当前码提前或者后移、删除的时候当前码会空余出来，自行选择哪个码按8提前来替换当前空码，之前自动以字母顺序顺延的逻辑感觉不太对。
...
- 所有关键 TSV 和 YAML 写入后都会立即重新读取校验；手机端写入失败时会显示错误。
