...
- 无候选时单击[开启自造词，自造词时**建议逐字打，打词也可，最好不要用630**，最后按空格确认上屏后自造词完成，自造词保存在pantsu.user.dict最后，需要重新部署才可以正常使用；
- 考虑到不需要太多候选，所以指定数字1-7为选字/词上屏，**8为将当前候选提前，9为将当前候选后移，0为将当前候选删除**，依然需要需要重新部署才可以正常使用。
- 打字时使用单引号'开关emoji；
- 按v开启临时英语；
