...
- 无候选时单击[开启自造词，**自造词时建议逐字打，打词也可，原则上可以但最好不要用630** ，最后按空格确认上屏后自造词完成，自造词保存在pantsu.zzc.dict.yaml最后，无需重新部署即可以直接使用；
...
### 自造词及调频功能详述
- 自造词前需知：  所有造出来的词会形成一个单独的文件，这个文件为： pantsu.zzc.dict.yaml。
- 自己的所有操作都在“pantsu_userdata/”文件夹下的tsv文件中。
- 空输入时按 `[`：按常规最短空码规则造词。
